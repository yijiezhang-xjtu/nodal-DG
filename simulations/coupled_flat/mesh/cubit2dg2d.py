...
class mesh():
    ...
    def elem_write(self,mesh_name):
        ...
        tri_vp=list(range(trilength))
        tri_vs=list(range(trilength))
        tri_rho=list(range(trilength))
        tri_qp=list(range(trilength))
        tri_qs=list(range(trilength))
        tri_block=list(range(trilength))
        for block,flag in zip(self.block_mat,self.block_flag):
            tris=cubit.get_block_tris(block)
            name=cubit.get_exodus_entity_name('block',block)
            type=cubit.get_block_element_type(block)
